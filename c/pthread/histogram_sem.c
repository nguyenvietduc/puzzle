...
// this program only uses mutex; semaphore is for experiemental purpose
...
typedef struct thread_args {
	...
	int chunk_size;			// size of elements to process
	int* starting_element;	// pointer to the first input of the thread
	int* histogram;			// pointer to the shared histogram
	int* buffer;			// local histogram buffer
} thread_args;
...
void compute_using_pthreads(int *input_data, int *histogram, int num_elements, int num_threads, int histogram_size)
{
	...
	int left_over;			// number of leftover elements at the end of input array
	thread_args* args;		// thread arguments to be passed to thread function
	pthread_t* threads;		// thread handles
	int** buffer;			// histogram buffers for child threads
